...
public class ServerGUI extends Application {
	...
	private HashMap<String, Scene> createServerScenes(Stage primaryStage) {
		...
		sceneMap.put("serverIntroScene", new Scene(serverIntroBox, 400, 400));
		
		//Server Game Scene
		Label serverGameLabel = new Label("Welcome to the Morra Server");
		Label connectedPlayersLabel = new Label("Players connected:");
		Label connectedGuestsLabel = new Label("Guests connected:");
		...
		VBox serverGameBox = new VBox(serverGameLabel, connectedBox, serverDialogueView);
		serverGameBox.getStyleClass().add("VBox");
		serverGameBox.setAlignment(Pos.CENTER);
		Scene serverGameScene = new Scene(serverGameBox, 400, 400);
